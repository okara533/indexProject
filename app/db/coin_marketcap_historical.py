...
from app.utils.helper import check_sqlite_connection,get_today_and_1year_ago_timestamps
...
if __name__ == "__main__":
    ...
    if pingCoinGeckoAPI():
        ...
        count = 1
        for id in coin_ids[500:]:
            print(count)
            logger.info(f"Processing coin ID: {id}")
            to_date,from_date = get_today_and_1year_ago_timestamps()
            data = histCoinData(id,vs_currency="usd",from_date=from_date,to_date=to_date)
